...
- Classification is grounded in the filesystem rather than path shape alone. A two-segment path is canonical only if it really is a git worktree root and no segment is a symlink. Without this, `<worktrees-root>/<flat-worktree>/<subdir>` — shape-identical to `<repo-name>/<worktree-name>` — let a single `cd` launder a forbidden flat worktree into a compliant one, and a symlinked segment could point a compliant-looking path at a shared checkout that rule 10 forbids mutating.
